...
const approvalSchema = new Schema(
  {
     ...
     * Links to the Request model (built by another team member).
     */
    requestId: {
     ...
     * Links to the User model (built by another team member).
     */
    approverId: {
      type: Schema.Types.ObjectId,
      ref: 'User',
     ...
     * Matches request.currentStep when the action was taken.
     ...
     * Must match roles used in roleMiddleware.js: manager, finance, director.
     */
    role: {
      type: String,
     ...
     * Matches the ACTIONS constants exported from workflowService.js.
     */
    action: {
      type: String,
     ...
     * Required for rejections (enforced by workflowService.processRejection).
     */
    comment: {
      type: String,
      default: '',
     ...
     * Set by workflowService.evaluateApprovalRule() during processApproval().
...
/**
 * Fast lookup of all approval records for a specific request, sorted by time.
 * Used to show the full audit trail for a request.
 */
approvalSchema.index({ requestId: 1, createdAt: 1 });

/**
 * Fast lookup of all actions taken by a specific approver, newest first.
 * Used to show "my approval history" views.
 */
